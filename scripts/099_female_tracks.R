...
#----------------------------------------------------------------------
# paper on male GPs: THIS IS THE females
#----------------------------------------------------------------------
unique(dat$sex)

dat <- subset(dat, dat$sex == "Female")
met <- subset(met, met$sex == "Female")
...
for (current_id in ids) {
  ...
  ggsave(plot = p3 ,
         bg = 'white',
         filename = paste0("./maps/female_tracks/", current_id, "_raw_locations_.png"),width=8,height=6)
  
}


#---------------------------
# Now only plot Marion area
#---------------------------
...
marion_bbox <- sf::st_as_sfc(sf::st_bbox(c(
  xmin = xmin, xmax = xmax, 
  ymin = ymin, ymax = ymax
), crs = 4326))
...
temp <- dat %>%
  ...
  sf_locs <- temp %>%
    ...
    sf::st_make_valid() %>%
    sf::st_crop(sf::st_bbox(marion_bbox))
  
  # Process sf_locs_nest
  sf_locs_nest <- temp %>%
    dplyr::select(deployment_decimal_longitude, deployment_decimal_latitude) %>%
    distinct() %>%
    sf::st_as_sf(coords = c("deployment_decimal_longitude", "deployment_decimal_latitude"), crs = 4326) %>%
    sf::st_make_valid() %>%
    sf::st_crop(sf::st_bbox(marion_bbox))
  ...
  p3 <- ggplot(data = island) +
    geom_sf(fill = "grey") +
  #  geom_sf(data = sf_lines, size = 0.75, color = "red") +
    geom_sf(data = sf_locs, size = 2, color = "red") +
    geom_sf(data = sf_locs_nest, size = 2, color = "blue") +
    coord_sf(xlim = c(xmin, xmax), ylim = c(ymin, ymax), expand = FALSE, clip = "on") +
    theme_bw() +
    labs(x = "Longitude", y = "Latitude") +
  ...
  ggsave(
    plot = p3, bg = "white",
    filename = paste0("./maps/female_tracks_marion/", current_id, "_raw_locations_.png"),
    width = 8, height = 6
  )
}
